...
const form = useForm({
    id: props.typeuser ? props.typeuser.id : "",
    name: props.typeuser ? props.typeuser.name : "",
    description: props.typeuser ? props.typeuser.description : "",
});

const submit = () => {
    if (props.typeuser) {
        form.put(route("typeusers.update", props.typeuser), {
            preserveScroll: true,
            onSuccess: () => emit("close-modal"),
        });
    } else {
        form.post(route("typeusers.store"), {
            preserveScroll: true,
            onSuccess: () => emit("close-modal"),
        });
    }
...
<template>
    <div class="flex justify-between bg-slate-300 h-12 px-4">
        <div class="text-lg sm:text-xl text-slate-500 font-bold inline-flex items-center">
            {{ form.id == 0 ? "Registrar Tipo de Usuario" : "Actualizar Tipo de Usuario" }}
        </div>
